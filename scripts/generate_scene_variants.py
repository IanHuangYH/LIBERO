...
NEW: Can create a new suite folder and generate variants there (instead of modifying original).
"""
...
# Import registration functions from create_new_libero_suite.py
script_dir = Path(__file__).parent
sys.path.insert(0, str(script_dir))
from create_new_libero_suite import update_task_map, update_benchmark_init, validate_suite
...
def create_suite_from_source(source_suite, new_suite):
    """
    Create a new suite by copying BDDL and init files from source suite.
    
    Args:
        source_suite: Name of source suite (e.g., 'libero_object')
        new_suite: Name of new suite (e.g., 'libero_object_variants')
        
    Returns:
        Tuple of (bddl_dir, init_dir) paths for the new suite
    """
    base_path = get_libero_base_path()
    
    source_bddl = base_path / "bddl_files" / source_suite
    target_bddl = base_path / "bddl_files" / new_suite
    
    source_init = base_path / "init_files" / source_suite
    target_init = base_path / "init_files" / new_suite
    
    print(f"\n📁 Creating new suite: {new_suite}")
    print(f"   Source: {source_suite}")
    
    # Create directories
    target_bddl.mkdir(parents=True, exist_ok=True)
    target_init.mkdir(parents=True, exist_ok=True)
    print(f"   ✅ Created directories")
    
    # Copy BDDL files (excluding variants)
    copied_bddl = 0
    for bddl_file in source_bddl.glob("*.bddl"):
        # Skip variant files (task_0.bddl, task_1.bddl, etc.)
        if re.match(r'.*_\d+\.bddl$', bddl_file.name):
            continue
        target_file = target_bddl / bddl_file.name
        shutil.copy2(bddl_file, target_file)
        copied_bddl += 1
    
    print(f"   ✅ Copied {copied_bddl} BDDL files")
    
    # Copy init files (excluding variants)
    copied_init = 0
    for init_file in source_init.glob("*.init"):
        # Skip variant files
        if re.match(r'.*_\d+\.init$', init_file.name):
            continue
        target_file = target_init / init_file.name
        shutil.copy2(init_file, target_file)
        copied_init += 1
    
    print(f"   ✅ Copied {copied_init} init files")
    print(f"   ✅ Suite created at:")
    print(f"      - {target_bddl}")
    print(f"      - {target_init}")
    
    return target_bddl, target_init
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  # Generate variants in original folder
  python generate_scene_variants.py --bddl task.bddl --init task.init --n_target_object 2 --n_target_location 2 --n_both_object_target 2
  
  # Create new suite and generate variants there
  python generate_scene_variants.py --create_new_suite libero_object_variants --copy_from libero_object --task pick_up_the_alphabet_soup_and_place_it_in_the_basket --n_target_object 2 --n_target_location 2 --n_both_object_target 2
  
  # Create suite for all tasks
  python generate_scene_variants.py --create_new_suite libero_object_variants --copy_from libero_object --all_tasks --n_target_object 2 --n_target_location 2 --n_both_object_target 2
        """,
        formatter_class=argparse.RawDescriptionHelpFormatter
    )
    
    # Mode 1: Work on specific files (original behavior)
    parser.add_argument("--bddl", type=str,
                       help="Path to original BDDL file")
    parser.add_argument("--init", type=str,
                       help="Path to original init states file")
    
    # Mode 2: Create new suite and generate variants
    parser.add_argument("--create_new_suite", type=str,
                       help="Create new suite with this name (e.g., libero_object_variants)")
    parser.add_argument("--copy_from", type=str,
                       help="Source suite to copy from (required with --create_new_suite)")
    parser.add_argument("--task", type=str,
                       help="Specific task to generate variants for (if not using --all_tasks)")
    parser.add_argument("--all_tasks", action="store_true",
                       help="Generate variants for all tasks in the suite")
    parser.add_argument("--skip_registration", action="store_true",
                       help="Skip automatic suite registration (advanced)")
    ...
    # Mode 2: Create new suite and generate variants
    if args.create_new_suite:
        if not args.copy_from:
            parser.error("--copy_from is required when using --create_new_suite")
        
        if not args.all_tasks and not args.task:
            parser.error("Must specify either --task or --all_tasks when using --create_new_suite")
        
        # Create the new suite
        bddl_dir, init_dir = create_suite_from_source(args.copy_from, args.create_new_suite)
        
        # Register the new suite automatically (unless skipped)
        if not args.skip_registration:
            print(f"\n📝 Registering new suite in LIBERO...")
            try:
                update_task_map(args.create_new_suite)
                update_benchmark_init(args.create_new_suite)
                print(f"   ✅ Suite registered successfully")
            except Exception as e:
                print(f"   ⚠️  Warning: Registration failed: {e}")
                print(f"   You may need to register manually in:")
                print(f"      - libero_suite_task_map.py")
                print(f"      - benchmark/__init__.py")
        else:
            print(f"\n⚠️  Skipping automatic registration (--skip_registration)")
            print(f"   Remember to register manually later!")
        
        # Get list of tasks to process
        if args.all_tasks:
            # Process all BDDL files in the new suite
            tasks = [f.stem for f in sorted(bddl_dir.glob("*.bddl"))]
        else:
            tasks = [args.task]
        
        print(f"\n🔄 Generating variants for {len(tasks)} task(s)...")
        
        # Process each task
        for task_name in tasks:
            bddl_path = bddl_dir / f"{task_name}.bddl"
            init_path = init_dir / f"{task_name}.init"
            
            if not bddl_path.exists():
                print(f"\n⚠️  Skipping {task_name}: BDDL file not found")
                continue
            
            if not init_path.exists():
                print(f"\n⚠️  Skipping {task_name}: init file not found")
                continue
            ...
            if args.output_dir:
                vis_dir = Path(args.output_dir) / task_name
            else:
                vis_dir = init_dir / "variant_visualizations" / task_name
        ...
        print(f"\n{'='*80}")
        print(f"✅ COMPLETED: Generated variants for {len(tasks)} task(s)")
        print(f"{'='*80}")
        print(f"\nNew suite location:")
        print(f"  BDDL: {bddl_dir}")
        print(f"  Init: {init_dir}")
        
        # Validate the suite (only if registered)
        if not args.skip_registration:
            print(f"\n🔍 Validating suite setup...")
            try:
                validate_suite(args.create_new_suite)
            except Exception as e:
                print(f"   ⚠️  Validation warning: {e}")
        
        print(f"\n✅ Suite is ready to use!")
        print(f"\nNext steps:")
        print(f"  1. Validate: bash scripts/validate_libero_suite.sh {args.create_new_suite}")
        print(f"  2. Test: TASK_SUITE={args.create_new_suite} ./eval_libero_quick_test.sh")
        print(f"{'='*80}\n")
    
    # Mode 1: Original behavior (work on specific files)
    else:
        if not args.bddl or not args.init:
            parser.error("--bddl and --init are required when not using --create_new_suite")
